...
export default class Category extends Component {
  ...
  initColumns = () => {
    this.columns = [
      ...
      },
      {
        ...
        key: 'x',
        width: 300,
        render: (category) => ( // 此处的category表示包含被点击行的所有数据的对象(只是一个对象而不是数组categorys)
          <span>
  ...
  // 显示子分类
  showSubCategorys = (category) => {  //点击查看子分类之后执行的函数
    const { _id, name } = category     //得到state中设置的parentName和parentID
    this.category = category
    this.setState({ parentId: _id, parentName: name },
      () => {
        // 该回调会在state和render之后触发(解决setstate异步更新的问题)
        this.getCategorys()      //展示二级分类-即为子分类
      })
  ...
  handleAddOk = async() => {
    ...
    // 1、获取数据
    const {parentId,categoryName} = this.form.current.getFieldsValue()  //这里的parentId,categoryName是在Addform组件中的Form.Item上的name属性值得到
    // 清除输入数据
    this.form.current.resetFields()
    // 判断一级分类还是二级分类;这里不用判断后端会根据传递的parentId值分辨是父分类还是子分类
    // const result = (await reqAddCategory(parentId,categoryName)).data
    // 2、发送请求（最重要的，使用接口，categoryName）
  ...
  render() {
    ...
    const category = this.category || {} // 如果还没有指定一个空对象，后续点击修改分类，其中默认名字因为找不到this.category会报错，因为category只有点击修改分类才会有而最开始render会开始执行一次，这个时候还没点击所以得不到category
